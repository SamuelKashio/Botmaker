...
# ✨ NUEVO: Import del módulo de análisis de chats (Opción A)
try:
    from chat_analyzer import render_chat_analysis
    HAS_CHAT_ANALYZER = True
except ImportError:
    HAS_CHAT_ANALYZER = False
...
# TABS
# ══════════════════════════════════════════════════════════
tab_rt, tab_sla, tab_team, tab_shifts, tab_alerts, tab_trends, tab_analysis = st.tabs([
    "🔴 Tiempo Real",
    "⚡ SLA & Tiempos",
    "👥 Equipo",
    "📋 Turnos",
    ...
    "📊 Tendencias",
    "🔍 Chat Analysis",  # ✨ NUEVA: Análisis de chats sin IA
])
...
with tab_analysis:
    ...
    st.subheader("🔍 Análisis de Chats - Tendencias de Contenido")
    
    st.markdown("""
    Detecta automáticamente las categorías, sentimientos y palabras clave más frecuentes
    en tus conversaciones. **100% gratis, sin necesidad de API externa.**
    
    ✨ Detecta: Categorías | Sentimientos | Palabras Clave | Velocidad de Resolución
    """)
    
    # Verificar si el módulo de análisis está disponible
    if not HAS_CHAT_ANALYZER:
        st.warning("⚠️ Para usar análisis de chats, instala las dependencias: `pip install textblob nltk scikit-learn`")
        st.info("📖 Una vez instaladas, recarga la página.")
    else:
        try:
            # Obtener datos de sesiones y mensajes
            with st.spinner("Obteniendo datos de chats..."):
                
                # Fetch sesiones
                status_sessions, sessions_data = api_get("sessions", {"limit": 500, "sort": "-createdAt"})
                
                if status_sessions != 200:
                    st.error(f"❌ Error al obtener sesiones: {status_sessions}")
                else:
                    df_sessions = pd.DataFrame(its(sessions_data))
                    
                    if df_sessions.empty:
                        st.info("📊 No hay sesiones disponibles en el período seleccionado.")
                    else:
                        # Obtener mensajes de cada sesión (limitar a 100 para no sobrecargar)
                        all_messages = []
                        progress_bar = st.progress(0)
                        session_ids = df_sessions['id'].tolist()[:100]
                        
                        for idx, sess_id in enumerate(session_ids):
                            try:
                                status_msgs, msgs_data = api_get(
                                    "messages",
                                    {"chat-id": sess_id, "long-term-search": "true"}
                                )
